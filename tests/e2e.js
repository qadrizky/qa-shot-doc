...
(function(){
  ...
  const autoBoxCount = () => frame().__qaShotDoc.S.items.reduce((n, it) => n + it.red.filter(b => b.auto).length, 0);

  // Cakupan kotak sensor terhadap ground truth, untuk satu pilihan checklist.
  function coverage(cats){
    const api = frame().__qaShotDoc, S = api.S;
    S.auto.cats = Object.fromEntries(Object.keys(S.auto.cats).map(k => [k, cats.includes(k)]));
    S.auto.applied = { ...S.auto.cats }; S.auto.on = true;
    api.applyAuto();
  ...
  test('e2e', 'Alur: screenshot baru tidak dipindai sebelum diminta', async () => {
    ...
    const scanned = frame().__qaShotDoc.S.items.filter(it => it._ocr).length;
    expect(scanned).toBe(1);
    await clickScan();
    expect(frame().__qaShotDoc.S.items.filter(it => it._ocr).length).toBe(fixtures.length);
  });
